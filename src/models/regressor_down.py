...
class StackingRegressorDown(BaseStackingModel):
    """
    Stacking Regressor for 하락 케이스 손실률 예측
    
    Model 3: 갭 상승 후 실제 하락한 케이스의 종가 기준 손실률 예측
    """
    ...
    def fit(
        ...
        Args:
            X: Feature DataFrame (하락 케이스만)
            y: Target Series (target_return, 음수)
            verbose: 상세 출력 여부
