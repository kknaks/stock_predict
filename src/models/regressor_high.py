"""
Model 2-1: Stacking Regressor (상승 시 고가 수익률 예측)

Base Learners: RandomForest, XGBoost, LightGBM
Meta Learner: Ridge

익절 전략용 - 고가 기준 최대 수익률 예측
"""
...
class StackingRegressorHigh(BaseStackingModel):
    """
    Stacking Regressor for 상승 케이스 고가 수익률 예측
    
    Model 2-1: 갭 상승 후 실제 상승한 케이스의 고가 기준 최대 수익률 예측
    익절 전략: 예측된 고가의 80% 지점에서 익절 주문
    """
    
    def __init__(
        self,
        ridge_alpha: float = 1.0,
        take_profit_ratio: float = 0.8,  # 익절 비율 (예: 80%)
        **kwargs
    ):
    ...
    def fit(
        ...
        Args:
            ...
            y: Target Series (target_max_return)
            verbose: 상세 출력 여부
        ...
        if verbose:
            print("=" * 80)
            print("Model 2-1: Stacking Regressor (상승 케이스 - 고가) 학습")
            print("=" * 80)
    ...
    def predict_take_profit(self, X: pd.DataFrame) -> np.ndarray:
        """
        익절 목표가 예측 (고가의 take_profit_ratio)
        ...
        Returns:
            익절 목표 수익률 (%)
        """
        max_return = self.predict(X)
        return max_return * self.take_profit_ratio
